...
import { getTrackers, getTrackerLogs, addTrackerLog, deleteTracker, getTrackerStreak } from '../tracker-lib'
  ...
  return (
    <nav className="border-b border-gray-200 bg-white px-6 py-3 flex items-center gap-6">
      ...
      <div className="flex gap-4 text-sm text-gray-500">
        ...
        <Link href="/hardwork" className="font-medium text-black">苦功夫</Link>
...
const CATEGORIES: Category[] = ['编程', '学习', '健身', '塑型', '写作', '其他']
...
export default function Home() {
  ...
  const [eventMins, setEventMins] = useState('')
  ...
  async function handleAddEvent(e: React.FormEvent) {
    e.preventDefault()
    const mins = parseInt(eventMins)
    if (!mins || !eventName.trim()) return
    setEventSaving(true)
    try {
      const r = await saveRecord({ date: today, category: '其他', minutes: mins, note: eventName.trim() })
      setRecords(prev => [...prev, r])
      setEventName(''); setEventMins(''); setEventOpen(false)
  ...
  // longest current streak across all trackers
  const longestStreak = trackersLoading || trackers.length === 0
    ? 0
    : Math.max(0, ...trackers.map(t => getTrackerStreak(trackerLogs, t)))
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-6 flex items-end justify-between">
          ...
          <div className="flex items-center gap-4 text-right">
            ...
            <div className="w-px h-8 bg-gray-100" />
            <div>
              <div className={`text-2xl font-black leading-none ${longestStreak >= 7 ? 'text-orange-500' : ''}`}>
                {trackersLoading || trackers.length === 0 ? '—' : longestStreak}
              </div>
              <div className="text-xs text-gray-400 mt-0.5">🔥 最长连胜</div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:grid lg:grid-cols-[1fr_380px] lg:gap-8 space-y-8 lg:space-y-0">
          ...
          <div>
            ...
            {/* 临时事件 */}
            <div className="mb-3">
              <button onClick={() => setEventOpen(x => !x)}
                className="w-full flex items-center justify-between px-4 py-3 border border-dashed border-gray-300 rounded-xl hover:border-gray-500 transition-colors text-sm">
                <span className="font-medium text-gray-500">＋ 临时事件</span>
                <span className="text-gray-300 text-xs">{eventOpen ? '收起 ▲' : '展开 ▼'}</span>
              </button>
              {eventOpen && (
                <form onSubmit={handleAddEvent} className="border border-dashed border-t-0 border-gray-300 rounded-b-xl px-4 pb-4 pt-3 -mt-1">
                  <input value={eventName} onChange={e => setEventName(e.target.value)}
                    placeholder="事件名称（如：攀岩、开会、散步）" required
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mb-2 focus:outline-none focus:border-black" />
                  <input type="number" value={eventMins} onChange={e => setEventMins(e.target.value)}
                    placeholder="时长（分钟）" min={1} required
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-black" />
                  <button type="submit" disabled={eventSaving || !eventName || !eventMins}
                    className="w-full mt-2 bg-gray-900 text-white rounded-lg py-2.5 text-sm font-bold hover:bg-black disabled:opacity-40">
                    {eventSaving ? '保存中...' : '记录'}
                  </button>
                </form>
              )}
