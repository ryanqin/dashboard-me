...
function ProgressRing({ pct, running, size = 40 }: { pct: number; running?: boolean; size?: number }) {
  ...
  const overshoot = pct >= 2
  const color = running ? '#16a34a' : overshoot ? '#f59e0b' : 'black'
  return (
...
function TrackerCard({ tracker, logs, onLog, onDelete }: {
  ...
  // 页面加载时检查：已暂停 + minutes 类型 + 已达目标 → 自动完成
  useEffect(() => {
    if (!timer || timer.running || tracker.unit !== 'minutes') return
    const elapsedMins = Math.max(1, Math.round(getElapsed(timer) / 60000))
    if (todayVal + elapsedMins >= tracker.dailyTarget) {
      handleFinish()
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [timer?.trackerId, tracker.unit, tracker.dailyTarget, todayVal])
  ...
  function handlePause() {
    ...
    window.dispatchEvent(new CustomEvent('timer-change', { detail: { trackerId: tracker.id } }))
    // 自动完成：minutes 类型追踪器，累计时长达到目标时自动记录
    if (tracker.unit === 'minutes') {
      const elapsedMins = Math.max(1, Math.round(s.elapsed / 60000))
      if (todayVal + elapsedMins >= tracker.dailyTarget) {
        handleFinish()
      }
    }
  ...
  return (
    <div className={`border rounded-xl transition-colors ${(tracker as {priority?:boolean}).priority ? 'border-amber-400 bg-amber-50/30' : done ? 'border-black' : isRunning ? 'border-green-400' : 'border-gray-200'}`}>
      <button onClick={() => setOpen(x => !x)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {(tracker as {priority?:boolean}).priority && <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium shrink-0">TOP</span>}
          </div>
          <div className="text-xs text-gray-400">
            {todayVal}/{tracker.dailyTarget} {UNIT_LABELS[tracker.unit]}{done && ' ✅'}
            {pct >= 2 && <span className="ml-1 text-amber-600 font-bold">✨{Math.floor(pct)}×</span>}
